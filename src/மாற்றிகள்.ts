...
import {
  base36,
  base36upper
} from "multiformats/bases/base36";
import {
  base58btc,
  base58flickr
...
export const __$மாற்றியை_பெறு = (
  { எழுத்து, குறிமுறை }: { எழுத்து: string, குறிமுறை: string }
): Codec<string, string> => {
  if (எழுத்து === "latin") {
    if (Object.keys(இலாத்தின்_அடைப்புகள்).includes(குறிமுறை))
      return இலாத்தின்_அடைப்புகள்[குறிமுறை as keyof typeof இலாத்தின்_அடைப்புகள்]
  ...
  if (!எழுத்து_அடைப்பு) throw `${எழுத்து} எழுத்துக்கு தெரியாத குறிமுறை ${குறிமுறை}`;
  ...
  const உருவாக்கி = baseX_அடைப்புகள்.find(இ => குறிமுறை.startsWith(இ)) ? baseX : rfc4648;

  return உருவாக்கி(வேண்டிய_அடைப்பு_தகவல்கள்)
}

export const கைரேகையை_பகுப்பாய்வு = (
  { கைரேகை }: { கைரேகை: string }
): { எழுத்து: string, குறிமுறை: string } => {
  const மூனோட்டு = [...கைரேகை][0];

  const இலாத்தின்_மாற்றி = Object.values(இலாத்தின்_அடைப்புகள்).find(இ => இ.prefix === மூனோட்டு);
  if (இலாத்தின்_மாற்றி) return { எழுத்து: "latin", குறிமுறை: இலாத்தின்_மாற்றி.name }

  for (const எ of எழுத்துகள்) {
    const மாற்றி_தகவல்கள் = Object.entries(எழுத்துகள்_அடைப்புகள்[எ]).find(இ => இ[1].முன்னோட்டு === மூனோட்டு)
